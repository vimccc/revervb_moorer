...
static inline float delay_step(rv_delay_t *delay, float v){
...
static inline float delay_get(rv_delay_t *delay, int offset){
...
static inline float iir1_step(rv_iir1_t *iir1, float v){
...
static inline void biquad_makeLPFQ(rv_biquad_t *biquad, int rate, float freq, float bw){
	freq = clampf(freq, 0, rate / 2);
	float omega = 2.0f * (float)M_PI * freq / (float)rate;
	float cs = cosf(omega);
...
static inline void biquad_makeAPF(rv_biquad_t *biquad, int rate, float freq, float bw){
	freq = clampf(freq, 0, rate / 2);
	float omega = 2.0f * (float)M_PI * freq / (float)rate;
	float sn = sinf(omega);
	float alpha = sn * sinhf((float)M_LN2 * 0.5f * bw * omega / sn);
...
static inline float biquad_step(rv_biquad_t *biquad, float v){
...
static inline sample_t earlyref_step(rv_earlyref_t *earlyref, sample_t input){
	static const sample_t gaintbl[18] = {
...
static inline float dccut_step(rv_dccut_t *dccut, float v){
...
static inline float lfo_step(rv_lfo_t *lfo){
	float v = lfo->im;
...
static inline float allpass1_step(rv_allpass1_t *allpass, float v){
...
static inline float allpass2_step(rv_allpass2_t *allpass2, float v){
...
static inline float allpass2_get1(rv_allpass2_t *allpass2, int offset){
...
static inline float allpass2_get2(rv_allpass2_t *allpass2, int offset){
...
static inline float allpass3_step(rv_allpass3_t *allpass3, float v, float mod){
...
static inline float allpass3_get1(rv_allpass3_t *allpass3, int offset){
...
static inline float allpass3_get2(rv_allpass3_t *allpass3, int offset){
...
static inline float allpass3_get3(rv_allpass3_t *allpass3, int offset){
...
static inline float allpassm_step(rv_allpassm_t *allpassm, float v, float mod, float fbmod){
...
static inline float comb_step(rv_comb_t *comb, float v, float feedback){
...
{2, 0.10f,-16.0f,-15, 1.0f, 0.1f, 1.0f, -5, 0.40f, 0.05f, 1.0f,18000, 100, 9000,18000,30.0f,0.000f}
	...
	#define CASE(prs, i)                                                                        \
		case prs: reverb_init(rv, rate, ps[i].osf, ps[i].p1, ps[i].p2, ps[i].p3, ps[i].p4, \
			ps[i].p5, ps[i].p6, ps[i].p7, ps[i].p8, ps[i].p9, ps[i].p10, ps[i].p11, ps[i].p12,  \
			ps[i].p13, ps[i].p14, ps[i].p15, ps[i].p16); return;
	switch (preset){
		CASE(REVERB_PRESET_DEFAULT    ,  0)
		CASE(REVERB_PRESET_SMALLHALL1 ,  1)
		CASE(REVERB_PRESET_SMALLHALL2 ,  2)
		CASE(REVERB_PRESET_MEDIUMHALL1,  3)
		CASE(REVERB_PRESET_MEDIUMHALL2,  4)
		CASE(REVERB_PRESET_LARGEHALL1 ,  5)
		CASE(REVERB_PRESET_LARGEHALL2 ,  6)
		CASE(REVERB_PRESET_SMALLROOM1 ,  7)
		CASE(REVERB_PRESET_SMALLROOM2 ,  8)
		CASE(REVERB_PRESET_MEDIUMROOM1,  9)
		CASE(REVERB_PRESET_MEDIUMROOM2, 10)
		CASE(REVERB_PRESET_LARGEROOM1 , 11)
		CASE(REVERB_PRESET_LARGEROOM2 , 12)
		CASE(REVERB_PRESET_MEDIUMER1  , 13)
		CASE(REVERB_PRESET_MEDIUMER2  , 14)
		CASE(REVERB_PRESET_PLATEHIGH  , 15)
		CASE(REVERB_PRESET_PLATELOW   , 16)
		CASE(REVERB_PRESET_LONGREVERB1, 17)
		CASE(REVERB_PRESET_LONGREVERB2, 18)
	}
	#undef CASE
}

void reverb_init(reverb_state_t *rv, int rate,
	...
	for (int i = 0; i < 4; i++){
		allpass1_make(&rv->crossL[i], nextprime(crossLc[i] * totfactor), 0.78f, 1);
		allpass1_make(&rv->crossR[i], nextprime(crossRc[i] * totfactor), 0.78f, 1);
	}
...
void reverb_process(reverb_state_t *rv, int size, sample_t *input, sample_t *output){
	...
	for (int i = 0; i < size; i++){
		// early reflection
		sample_t er = earlyref_step(&rv->earlyref, input[i]);
		...
		for (int i2 = 0; i2 < rv->oversampleL.factor; i2++){
			...
			for (int i = 0; i < 4; i++){
				crossL = allpass1_step(&rv->crossL[i], crossL);
				crossR = allpass1_step(&rv->crossR[i], crossR);
			}
